...
const WhatIdo = () => {
  ...
  return (
    <>
      <section id='Skills' className='mx-4 lg:mx-20 pt-5  '>
        ...
        <div className='flex justify-center   gap-y-8    flex-col  w-full  '>
          ...
          {
            services.map((el) => <div key={el.id} className={`  flex-1  lg:px-10  flex flex-col  text-white    `}>
               {/* {i % 2 === 0 ?
                <Section><Text title={el.title} text={el.text} /> </Section>
                :
                 <SectionLeft><Text title={el.title} text={el.text} /> </SectionLeft>
                  ...
                  <p className={`handFont  text-[12px] md:text-sm lg:text-base  lg:mx-1 mt-4 lg:mt-10 text-center  `}>
                    {el.text}
                  </p>
